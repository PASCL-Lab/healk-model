...
def calculate_map(predictions, ground_truths, iou_threshold=0.5):
    ...
    classes = list(set([gt['labels'] for gt in ground_truths]))

    for c in classes:
        # Get predictions and ground truths for class `c`
        preds_class = [p for p in predictions if p['labels'] == c]
        gts_class = [gt for gt in ground_truths if gt['labels'] == c]
...
def validate(valid_data_loader, model):
    ...
    for i, data in enumerate(prog_bar):
        ...
        # true_boxes = targets[0]['boxes']  # Bounding boxes in (x_min, y_min, x_max, y_max) format
...
# print(model_predictions[0], ground_truths[0])
...
# accuracy_predictions =  torch.as_tensor([item['labels'].item() for item in model_predictions], dtype=torch.int64)
# accuracy_targets =torch.as_tensor([item['labels'].item() for item in ground_truths], dtype=torch.int64)

# print(accuracy_predictions, accuracy_targets)

# model_accuracy = accuracy(accuracy_predictions, accuracy_targets)
